...
func (g GenericValue) String() string {
	var cstr *C.char
	cstr = C.LLVMGenericValueToString(g.C)
...
func NewMCJITCompilerWithTargetMachine(m Module, tm TargetMachine) (ee ExecutionEngine, err error) {
	var cmsg *C.char
	fail := C.LLVMCreateMCJITCompilerWithTMForModule(&ee.C, m.C, tm.C, &cmsg)
	if fail != 0 {
		ee.C = nil
		err = errors.New(C.GoString(cmsg))
		C.LLVMDisposeMessage(cmsg)
...
func (ee ExecutionEngine) EmitMCObject(m Module) (buf MemoryBuffer, ok bool) {
	fail := C.LLVMMCJITEmitMCObject(ee.C, m.C, &buf.C)
	return buf, fail == 0
}

func LoadLibraryPermanentlyNull() {
	C.LLVMLoadLibraryPermanentlyNull()
}

func (g GenericValue) VoidPointer() unsafe.Pointer {
	return C.LLVMGenericValueToVoidPointer(g.C)
}

func (ee ExecutionEngine) GetExecutionEngineTargetMachine() (tm TargetMachine) {
	tm.C = C.LLVMGetExecutionEngineTargetMachine(ee.C)
	return
}
