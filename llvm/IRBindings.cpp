...
LLVMBool LLVMMaterializeMetadata(LLVMModuleRef M) {
  if (auto E = unwrap(M)->materializeMetadata()) {
    fprintf(stderr, "LLVMMaterializeMetadata error!\n");
    logAllUnhandledErrors(std::move(E), llvm::errs(), "error: ");
    return 1;
  }
...
LLVMBool LLVMMaterialize(LLVMModuleRef M, LLVMValueRef V) {
  auto gv = (GlobalValue *)(unwrap(V));
  if (auto E = unwrap(M)->materialize(gv)) {
    fprintf(stderr, "LLVMMaterialize error!\n");
    logAllUnhandledErrors(std::move(E), llvm::errs(), "error: ");
    return 1;
  }

  if (gv->isDeclaration()) {
    fprintf(stderr, "LLVMMaterialize value is declaration!\n");
    return 1;
  }
  if (!gv->hasExternalLinkage()) {
    fprintf(stderr, "LLVMMaterialize value does not have external linkage!\n");
    return 1;
  }
...
void LLVMMarkFastMathFlags(LLVMValueRef Inst) {
