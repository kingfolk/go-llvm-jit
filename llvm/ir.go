...
func (m Module) GetNamedFunction(name string) (t Value) {
	cname := C.CString(name)
	defer C.free(unsafe.Pointer(cname))
	t.C = C.LLVMGetNamedFunction(m.C, cname)
	return
}

func (m Module) GetSourceFileName() string {
	var l uint
	size := C.size_t(l)
	cname := C.LLVMGetSourceFileName(m.C, &size)
	return C.GoString(cname)
}

func (b Builder) CreateMemCpy(dst, src, size Value, dstAlign, srcAlign int) (v Value) {
	v.C = C.LLVMBuildMemCpy(b.C, dst.C, C.uint(dstAlign), src.C, C.uint(srcAlign), size.C)
	return
}

func (b Builder) CreateFAddFast(lhs, rhs Value, name string) (v Value) {
	cname := C.CString(name)
	defer C.free(unsafe.Pointer(cname))
	v.C = C.LLVMBuildFAdd(b.C, lhs.C, rhs.C, cname)
	return
...
func (v Value) MarkFastMathFlags() {
	C.LLVMMarkFastMathFlags(v.C)
}
