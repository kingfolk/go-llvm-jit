...
func (tm TargetMachine) CPU() string {
	cstr := C.LLVMGetTargetMachineCPU(tm.C)
	return C.GoString(cstr)
}

func (tm TargetMachine) Features() string {
	cstr := C.LLVMGetTargetMachineFeatureString(tm.C)
	return C.GoString(cstr)
}
...
func HostCPUFeatures() (feature string) {
	cstr := C.LLVMGetHostCPUFeatures()
	defer C.free(unsafe.Pointer(cstr))
	feature = C.GoString(cstr)
	return
}

func HostCPUName() (cpu string) {
	cstr := C.LLVMGetHostCPUName()
	defer C.free(unsafe.Pointer(cstr))
	cpu = C.GoString(cstr)
	return
}
...
func (tm TargetMachine) EmitMCObject(m Module) (buf MemoryBuffer, ok bool) {
	fail := C.LLVMTMEmitMCObject(tm.C, m.C, &buf.C)
	return buf, fail == 0
}
